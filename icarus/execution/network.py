...
class NetworkController(object):
    ...
    def put_content(self, node):
        ...
        """
        if node in self.model.cache:
            evicted = self.model.cache[node].put(self.session['content'])
            if evicted:
                self.cache_eviction(node)
            return evicted
    ...
    def remove_content(self, node):
        ...
        """
        if node in self.model.cache:
            removed = self.model.cache[node].remove(self.session['content'])
            if removed:
            	self.collector.cache_evict(node)
            return removed
    
    def cache_eviction(self, node):
    	self.collector.cache_evict(node)
    	return True
