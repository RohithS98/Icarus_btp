...
class DataCollector(object):
    ...
    def cache_evict(self, node):
        """Reports that some data at the cache at node *node* has been
        evicted from the cache
		
        Parameters
        ----------
        node : any hashable type
            The node from which the content was evicted
        """
        pass
...
class CollectorProxy(DataCollector):
    ...
    EVENTS = ('start_session', 'end_session', 'cache_hit', 'cache_miss', 'cache_evict', 'server_hit', 'request_hop', 'content_hop', 'results')
...
@register_data_collector('EVICTIONS')
class EvictionCollector(DataCollector):
    """Data collector measuring number of evictions
    """
    ...
    def cache_evict(self, node):
        self.evictions += 1
        #print(self.evictions)

    @inheritdoc(DataCollector)
    def results(self):
        print('Evict:',self.evictions)
        results = Tree({'NUMBER': self.evictions})
        return results


@register_data_collector('UTILIZATION')
class UtilizationCollector(DataCollector):
    """Data collector measuring cache utilization
    """
    ...
    def __init__(self, view):
        ...
        self.view = view
        self.req = 0
        self.collectInterval = 100
        self.nodes = self.view.cache_nodes()
        self.utils = []

    @inheritdoc(DataCollector)
    def start_session(self, timestamp, receiver, content):
        self.req += 1
        if self.req % self.collectInterval == 0:
        	total = 0
        	used = 0
        	for node in self.nodes:
        	    cacheT = self.view.model.cache[node]
        	    total += cacheT.maxlen
        	    used += len(cacheT)
        	self.utils.append((used/total))

    @inheritdoc(DataCollector)
    def results(self):
        #print(self.utils)
        results = Tree({'VALUES': self.utils,
        				'MEAN'	: sum(self.utils)/len(self.utils)})
        return results

@register_data_collector('DIVERSITY')
class UtilizationCollector(DataCollector):
    """Data collector measuring cache diversity
    """
    ...
    def start_session(self, timestamp, receiver, content):
        ...
        if self.req % self.collectInterval == 0:
        	...
        	for node in self.nodes:
        	    cacheT = self.view.model.cache[node]
        	    used += len(cacheT)
        	    content.update(cacheT.dump())
        	self.divs.append(len(content)/used)

    @inheritdoc(DataCollector)
    def results(self):
        #print(self.divs)
        results = Tree({'VALUES': self.divs,
        				'MEAN'	: sum(self.divs)/len(self.divs)})
        return results
